...
import com.maxxton.microdocs.crawler.spring.config.ConfigBuilder;
...
public class DocletRunner extends Standard {
    ...
    public static boolean start(RootDoc root) {
        ...
        //create CrawlerConfiguration
        Config crawlerConfiguration = new ConfigBuilder()
                .eventProducerType(config.getEventProducerType())
                .eventPublisherMethodCall(config.getEventProducerPublisher())
                .pathToProjectSrcFolder(config.getProjectSourcePath())
                .pathToApplicationYamlFile(config.getYamlPath())
                .pathToEventConsumerBindingKeys(config.getYamlPathToEventConsumerKeys())
                .pathToEventConsumerQueueName(config.getYamlPathToEventConsumerName())
                .addedPrefixToEventConsumerQueueName(config.getAddedPrefixToEventConsumerQueueName())
                .build();

        // run crawler
        Project project = crawler.crawl(classes, crawlerConfiguration);
